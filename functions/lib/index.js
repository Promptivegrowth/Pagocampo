...
exports.confirmSent = exports.api = void 0;
...
// Secrets
// -----------------------------------------------------------------------------
const S_TWILIO_SID = (0, params_1.defineSecret)("TWILIO_ACCOUNT_SID");
const S_TWILIO_TOKEN = (0, params_1.defineSecret)("TWILIO_AUTH_TOKEN");
const S_TWILIO_MSG_SERVICE_SID = (0, params_1.defineSecret)("TWILIO_MSG_SERVICE_SID");
const S_TWILIO_VIRTUAL_TO = (0, params_1.defineSecret)("TWILIO_VIRTUAL_TO");
const S_LIGHTHOUSE_KEY = (0, params_1.defineSecret)("LIGHTHOUSE_API_KEY");
const S_RPC = (0, params_1.defineSecret)("ANCHOR_RPC");
const S_PK = (0, params_1.defineSecret)("ANCHOR_PK");
const S_ADDR = (0, params_1.defineSecret)("ANCHOR_ADDR");
const S_BENEFICIARY = (0, params_1.defineSecret)("ANCHOR_BENEFICIARY");
...
// Helpers
// -----------------------------------------------------------------------------
const sha256hex = (s) => "0x" + (0, crypto_1.createHash)("sha256").update(s).digest("hex");
const toCents = (s) => Math.round(parseFloat(String(s).replace(",", ".")) * 100);
function makeTwilio() {
...
// -----------------------------------------------------------------------------
async function anchorOnPolygon(cid, intentId, spaceDid) {
    const rpc = S_RPC.value();
    const pk = S_PK.value();
    const addr = S_ADDR.value();
    if (!rpc || !pk || !addr) {
        console.warn("⚠ Missing blockchain secrets, returning mock txHash (solo demo).");
        return "0x" + (0, crypto_1.createHash)("sha256").update(cid + intentId + Date.now()).digest("hex");
    }
    ...
    const beneficiary = ethers.getAddress(S_BENEFICIARY.value()?.trim() || sender);
    ...
    try {
        ...
        if (isPaused)
            throw Object.assign(new Error("PAUSED"), { reason: "PAUSED" });
    }
    catch (e) {
        if (e?.reason === "PAUSED" || e?.message === "PAUSED")
            throw e;
    ...
    try {
        ...
        if (expTs === 0n) {
            try {
                const bal = await lock.balanceOf(beneficiary);
                if (bal > 0n) {
                    const tokenId = await lock.tokenOfOwnerByIndex(beneficiary, 0);
        ...
        if (e?.reason === "NO_VALID_UNLOCK_KEY" || e?.message === "NO_VALID_UNLOCK_KEY")
            throw e;
    ...
    const decodeRevert = (err) => {
        try {
            ...
            if (err?.data) {
                ...
                if (parsed?.name)
                    return `error: ${parsed.name} args=${JSON.stringify(parsed.args)}`;
            }
        }
        catch { }
    ...
    const fn = contract.getFunction("anchorReceipt");
...
// -----------------------------------------------------------------------------
async function sendToVirtualPhone(twilio, msgSid, virtualTo, body) {
...
app.post("/intents", async (req, res) => {
    try {
        const { phone, amount, code, payerName, toName, toPhone, toAddr, note } = req.body;
        ...
        const text = `PagoCampo: ${whoPays} pagará S/ ${amount}${whoGets}. ` +
...
app.post("/sms/inbound", async (req, res) => {
    try {
        ...
        const it = (snap.exists ? snap.data() : {});
        const payeePhone = it?.to || null;
        const payeeMsisdnHash = it?.payeeMsisdnHash || (payeePhone ? sha256hex(payeePhone) : null);
        const toAddr = it?.toAddr || null;
        await intentRef.set({
            intentId,
            amount: amountCents,
            from,
...
app.post("/admin/confirm-all", async (_req, res) => {
    try {
        ...
        await sendToVirtualPhone(twilio, msgSid, virtualTo, "✅ Confirmación manual enviada.");
...
app.get("/admin/filecoin/deals/:cid", async (req, res) => {
    try {
        ...
        await db.collection("filecoinDeals").doc(cid).set({ deals, updatedAt: Date.now() }, { merge: true });
...
// -----------------------------------------------------------------------------
exports.confirmSent = (0, scheduler_1.onSchedule)({
